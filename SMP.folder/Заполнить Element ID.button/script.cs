...
for(int i=0; i<objects.Count; i++) {
    ...
    if(props != null) {
        ...
        if(p != null) {
            string expectedId = obj.Id.ToString();
            if (p.GetStringValue() != expectedId) {
                p.SetStringValue(expectedId); 
                ok++; 
            }
        }
    }
...
System.Windows.Forms.MessageBox.Show(
    $"Свойство {propName} успешно присвоено всем {ok} элементам проекта.", 
